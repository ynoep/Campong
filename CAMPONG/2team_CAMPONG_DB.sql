...
insert into `MEMBER`(`USER_NO`,`ID`,`PASSWORD`,`ROLE`,`BIRTHYEAR`,`NICKNAME`,`PHONE`,`EMAIL1`,`EMAIL2`,`ADDRESS`,`KAKAOTOKEN`,`STATUS`,`ENROLL_DATE`,`MODIFY_DATE`,`PROFILE_IMAGE`) 
values(0,'admin','1212','ROLE_ADMIN','1970','관리자','01012345678','admin','ruu.kr','서울','',DEFAULT,DEFAULT,DEFAULT,'');
...
SELECT COUNT(*) FROM MEETINGMEMBER WHERE MEET_NO=5 AND USER_NO=1;
...
CREATE TABLE `BUS_TICKET` (
	`USER_NO`	INT NOT NULL,
	`STARTSTATIONID`	VARCHAR(100),
	`ENDSTATIONID`	VARCHAR(100),
	`PRICE`	INT,
	`TIME`	VARCHAR(100),
	`TAKENTIME`	VARCHAR(100),
	`APES`	INT NOT NULL
);
...
ALTER TABLE `BOOKMARK` ADD CONSTRAINT `FK_CAMPING_CONTENTS_TO_BOOKMARK_1` FOREIGN KEY (
	`CONTENTID`
)
REFERENCES `CAMPING_CONTENTS` (
	`CONTENTID`
...
ALTER TABLE `BUS_TICKET` ADD CONSTRAINT `FK_MEMBER_TO_BUS_TICKET_1` FOREIGN KEY (
	`USER_NO`
)
REFERENCES `MEMBER` (
	`USER_NO`
...
ALTER TABLE `TRAIN_TICKET` ADD CONSTRAINT `FK_MEMBER_TO_TRAIN_TICKET_1` FOREIGN KEY (
	`USER_NO`
)
REFERENCES `MEMBER` (
	`USER_NO`
...
drop table CAMPING_CONTENTS_REPLY;
...
-- 추가한 부분
ALTER TABLE `CAMPING_CONTENTS` ADD `READCOUNT` INT DEFAULT 0;
ALTER TABLE `CAMPING_CONTENTS` ADD `REVIEWCOUNT` INT DEFAULT 0;
ALTER TABLE `CAMPING_CONTENTS` ADD `BOOKMARKCOUNT` INT DEFAULT 0;
